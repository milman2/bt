...
namespace action
{
    ...
    NodeStatus TeleportToNearest::Execute(Context& context)
    {
        ...
        if (teleport_success)
        {
            std::cout << "TeleportToNearest 액션: 텔레포트 성공 - 플레이어 " << client_executor->GetName() << std::endl;
            return NodeStatus::SUCCESS;
        }
        else
        {
            std::cout << "TeleportToNearest 액션: 텔레포트 실패 - 플레이어 " << client_executor->GetName() << std::endl;
            return NodeStatus::FAILURE;
        }
    }
