    ...
    namespace condition
    {
        ...
        NodeStatus TeleportTimer::Execute(Context& context)
        {
            ...
            if (!client_executor->HasTarget() ||
                client_executor->GetDistanceToTarget() > client_executor->GetDetectionRange())
            {
                // 텔레포트 타이머 가져오기
                ...
                {
                    std::cout << "TeleportTimer 조건: 텔레포트 타이머 만료 (" << teleport_timer
                              << "초 >= " << TELEPORT_TIMEOUT << "초)" << std::endl;
                    return NodeStatus::SUCCESS; // 텔레포트 실행 필요
                }
                else
                {
                    std::cout << "TeleportTimer 조건: 텔레포트 타이머 진행 중 (" << teleport_timer << "초 / "
                              << TELEPORT_TIMEOUT << "초)" << std::endl;
