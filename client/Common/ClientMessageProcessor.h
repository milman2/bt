...
namespace bt
{
    ...
    public:
        ...
        PlayerActionMessage(ActionType action, float x = 0, float y = 0, float z = 0, uint32_t target_id = 0)
            : ClientMessage(ClientMessageType::PLAYER_ACTION_REQUEST)
            , action_(action)
            , x_(x)
            , y_(y)
            , z_(z)
    ...
    public:
        ...
        const std::unordered_map<uint32_t, std::tuple<float, float, float, float>>& GetMonsters() const
        {
            return monsters_;
        }
    ...
    public:
        CombatResultMessage(uint32_t attacker_id, uint32_t target_id, uint32_t damage, uint32_t remaining_health)
            : ClientMessage(ClientMessageType::COMBAT_RESULT)
            , attacker_id_(attacker_id)
            , target_id_(target_id)
            , damage_(damage)
            , remaining_health_(remaining_health)
        {
        }
