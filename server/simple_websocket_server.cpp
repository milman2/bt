...
void SimpleWebSocketServer::handle_client(int client_socket) {
    ...
    if (bytes_read > 0) {
        ...
        if (perform_handshake(client_socket, request)) {
            // 세션 생성 및 추가
            auto session = std::make_shared<SimpleWebSocketSession>(client_socket, SimpleWebSocketSession::get_next_session_id());
            {
                std::lock_guard<std::mutex> lock(sessions_mutex_);
            ...
            // 환영 메시지 전송
            std::string welcome_msg = R"({"type":"system_message","data":{"message":"WebSocket 서버에 연결되었습니다.","level":"info"},"timestamp":)" + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count()) + "}";
            ...
            while (session->is_connected()) {
                ...
                // 간단한 echo 응답
                std::string response = R"({"type":"echo","data":{"message":"메시지를 받았습니다."},"timestamp":)" + std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count()) + "}";
    ...
    while (std::getline(request_stream, line)) {
        if (line.find("Sec-WebSocket-Key:") != std::string::npos) {
            ...
            if (pos != std::string::npos) {
                ...
                // 개행 문자 제거
                websocket_key.erase(std::remove(websocket_key.begin(), websocket_key.end(), '\r'), websocket_key.end());
            }
            break;
        }
    }
