...
void MessageBasedMonsterManager::AddMonster(std::shared_ptr<Monster> monster)
{
    ...
    SendMonsterSpawnMessage(id, monster->GetName(), 
                           MonsterFactory::MonsterTypeToString(monster->GetType()), position,
                           monster->GetStats().health, monster->GetStats().max_health);
...
std::shared_ptr<Monster> MessageBasedMonsterManager::SpawnMonster(
    MonsterType type, const std::string& name, const MonsterPosition& position)
{
...
void MessageBasedMonsterManager::AddSpawnConfig(const MonsterSpawnConfig& config)
{
    ...
    std::cout << "스폰 설정 추가됨: " << config.name << " (" << 
                 MonsterFactory::MonsterTypeToString(config.type) << ")" << std::endl;
}

void MessageBasedMonsterManager::RemoveSpawnConfig(MonsterType type, const std::string& name)
{
    auto it = std::find_if(spawn_configs_.begin(), spawn_configs_.end(),
...
void MessageBasedMonsterManager::ProcessAutoSpawn(float delta_time)
{
    ...
    for (const auto& config : spawn_configs_) {
        if (!config.auto_spawn) continue;
        
        // 현재 타입의 몬스터 수 확인
        size_t current_count = GetMonsterCountByType(config.type);
        if (current_count >= config.max_count) continue;
        ...
        if (last_spawn_it != last_spawn_times_.end()) {
            auto time_since_last = std::chrono::duration_cast<std::chrono::seconds>(
                now - last_spawn_it->second).count();
        ...
        std::cout << "자동 스폰: " << config.name << " at (" << 
                     spawn_pos.x << ", " << spawn_pos.z << ")" << std::endl;
    }
}
...
void MessageBasedMonsterManager::SendMonsterSpawnMessage(uint32_t monster_id, const std::string& name, 
                                                        const std::string& type, const MonsterPosition& position,
                                                        float health, float max_health)
{
    if (!message_processor_) return;
    
    auto message = std::make_shared<MonsterSpawnMessage>(
        monster_id, name, type, position.x, position.y, position.z, position.rotation,
        health, max_health);
    
    message_processor_->SendToNetwork(message);
}

void MessageBasedMonsterManager::SendMonsterMoveMessage(uint32_t monster_id, const MonsterPosition& from, 
                                                       const MonsterPosition& to)
{
    if (!message_processor_) return;
    
    auto message = std::make_shared<MonsterMoveMessage>(
        monster_id, from.x, from.y, from.z, to.x, to.y, to.z);
    
    message_processor_->SendToNetwork(message);
}

void MessageBasedMonsterManager::SendMonsterDeathMessage(uint32_t monster_id, const std::string& name, 
                                                        const MonsterPosition& position)
{
    if (!message_processor_) return;
    
    auto message = std::make_shared<MonsterDeathMessage>(
