...
private:
    ...
    // 메시지 전송 헬퍼
    void SendMonsterSpawnMessage(uint32_t monster_id, const std::string& name, 
                                const std::string& type, const MonsterPosition& position,
                                float health, float max_health);
    void SendMonsterMoveMessage(uint32_t monster_id, const MonsterPosition& from, 
                               const MonsterPosition& to);
    void SendMonsterDeathMessage(uint32_t monster_id, const std::string& name, 
                                const MonsterPosition& position);
};
