...
namespace bt
{
    ...
    MessageBasedPlayerManager::MessageBasedPlayerManager() : next_player_id_(1), running_(false) {}
    ...
    void MessageBasedPlayerManager::AddPlayer(std::shared_ptr<Player> player)
    {
        ...
        std::cout << "MessageBasedPlayerManager: 플레이어 추가됨 - ID: " << player_id << ", 이름: " << player->GetName()
                  << std::endl;

        // 플레이어 조인 메시지 전송
        auto pos = player->GetPosition();
        auto join_message =
            std::make_shared<PlayerJoinMessage>(player_id, player->GetName(), pos.x, pos.y, pos.z, pos.rotation);
    ...
    std::vector<std::shared_ptr<Player>> MessageBasedPlayerManager::GetPlayersInRange(float x,
                                                                                      float y,
                                                                                      float z,
                                                                                      float range)
    {
        return players_.filter(
            [x, y, z, range](const std::shared_ptr<Player>& player)
            {
                if (!player)
                    return false;

                auto  pos      = player->GetPosition();
                float distance = std::sqrt(std::pow(pos.x - x, 2) + std::pow(pos.y - y, 2) + std::pow(pos.z - z, 2));
    ...
    std::shared_ptr<Player> MessageBasedPlayerManager::CreatePlayer(const std::string&     name,
                                                                    const MonsterPosition& position)
    {
        ...
        // 플레이어 생성 메시지 전송
        auto join_message =
            std::make_shared<PlayerJoinMessage>(player_id, name, position.x, position.y, position.z, position.rotation);
        message_processor_->SendToNetwork(join_message);

        return player;
    }

    std::shared_ptr<Player> MessageBasedPlayerManager::CreatePlayerForClient(uint32_t               client_id,
    ...
    void MessageBasedPlayerManager::HandlePlayerJoinMessage(std::shared_ptr<PlayerJoinMessage> message)
    {
        ...
        std::cout << "MessageBasedPlayerManager: 플레이어 조인 메시지 처리 - ID: " << message->player_id
                  << ", 이름: " << message->player_name << std::endl;
