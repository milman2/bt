...
public:
    GameMessage(MessageType type, const std::string& source = "")
        : type_(type), source_(source), timestamp_(std::chrono::duration_cast<std::chrono::milliseconds>(
...
public:
    ...
    MonsterSpawnMessage(uint32_t id, const std::string& name, const std::string& type,
                       float x, float y, float z, float rotation,
                       float health, float max_health)
        : GameMessage(MessageType::MONSTER_SPAWN, "MonsterManager"),
          monster_id(id), monster_name(name), monster_type(type),
    ...
    nlohmann::json ToJson() const override {
        ...
        json["position"] = {{"x", x}, {"y", y}, {"z", z}, {"rotation", rotation}};
        json["health"] = health;
...
public:
    ...
    MonsterMoveMessage(uint32_t id, float from_x, float from_y, float from_z,
                      float to_x, float to_y, float to_z)
        : GameMessage(MessageType::MONSTER_MOVE, "MonsterManager"),
          monster_id(id), from_x(from_x), from_y(from_y), from_z(from_z),
          to_x(to_x), to_y(to_y), to_z(to_z) {}
    
    nlohmann::json ToJson() const override {
        auto json = GameMessage::ToJson();
        json["monster_id"] = monster_id;
        json["from"] = {{"x", from_x}, {"y", from_y}, {"z", from_z}};
        json["to"] = {{"x", to_x}, {"y", to_y}, {"z", to_z}};
        return json;
    }
};
...
public:
    ...
    MonsterDeathMessage(uint32_t id, const std::string& name, float x, float y, float z)
        : GameMessage(MessageType::MONSTER_DEATH, "MonsterManager"),
          monster_id(id), monster_name(name), x(x), y(y), z(z) {}
...
public:
    ...
    PlayerJoinMessage(uint32_t id, const std::string& name, float x, float y, float z, float rotation)
        : GameMessage(MessageType::PLAYER_JOIN, "PlayerManager"),
          player_id(id), player_name(name), x(x), y(y), z(z), rotation(rotation) {}
...
public:
    ...
    PlayerMoveMessage(uint32_t id, float from_x, float from_y, float from_z,
                     float to_x, float to_y, float to_z)
        : GameMessage(MessageType::PLAYER_MOVE, "PlayerManager"),
          player_id(id), from_x(from_x), from_y(from_y), from_z(from_z),
          to_x(to_x), to_y(to_y), to_z(to_z) {}
...
public:
    ...
    GameStateUpdateMessage(const nlohmann::json& state)
        : GameMessage(MessageType::GAME_STATE_UPDATE, "GameEngine"),
          game_state(state) {}
...
public:
    ...
    NetworkBroadcastMessage(const std::string& event_type, const nlohmann::json& data)
        : GameMessage(MessageType::NETWORK_BROADCAST, "NetworkManager"),
...
public:
    ...
    SystemShutdownMessage(const std::string& reason = "")
        : GameMessage(MessageType::SYSTEM_SHUTDOWN, "System"),
          reason(reason) {}
