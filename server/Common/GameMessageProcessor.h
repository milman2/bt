...
namespace bt
{
    ...
    public:
        ...
        // 게임 로직에 메시지 전송
        void SendToGame(std::shared_ptr<GameMessage> message) { game_queue_.Push(message); }
        ...
        // 네트워크에 메시지 전송
        void SendToNetwork(std::shared_ptr<GameMessage> message) { network_queue_.Push(message); }
