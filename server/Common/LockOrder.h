...
namespace bt
{
    ...
    public:
        static void SetLastLockedOrder(int order) { last_locked_order_ = order; }

        static int GetLastLockedOrder() { return last_locked_order_; }

        static void Reset() { last_locked_order_ = 0; }
    };
    ...
    {                                                                                                          \
        ...
        {                                                                                                      \
            std::cerr << "Lock order violation detected! Expected order > "                                    \
                      << bt::LockOrderTracker::GetLastLockedOrder() << ", but got " << static_cast<int>(order) \
