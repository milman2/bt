...
AsioServer::AsioServer(const AsioServerConfig& config) 
    : config_(config), running_(false), acceptor_(io_context_), 
      total_packets_sent_(0), total_packets_received_(0) {
...
void AsioServer::handle_accept(boost::shared_ptr<AsioClient> client, const boost::system::error_code& error) {
    ...
    if (!error) {
        log_message("클라이언트 연결 성공, IP: " + client->get_ip_address() + ":" + std::to_string(client->get_port()));
        ...
        log_message("새 클라이언트 연결 완료: " + client->get_ip_address() + ":" + std::to_string(client->get_port()));
    } else {
        if (running_.load()) {
...
void AsioServer::add_client(boost::shared_ptr<AsioClient> client) {
    ...
    if (player_manager_) {
        ...
        if (player) {
            ...
            log_message("클라이언트 연결: " + client_info + " -> 플레이어 생성: " + player_name + " (ID: " + std::to_string(player->get_id()) + ")");
        }
    }
}
...
AsioClient::AsioClient(boost::asio::io_context& io_context, AsioServer* server)
    : io_context_(io_context), socket_(io_context), server_(server), 
      connected_(false), expected_packet_size_(0), sending_(false) {
