...
// BTNode 기본 구현
BTNode::BTNode(const std::string& name, BTNodeType type) 
...
BTSequence::BTSequence(const std::string& name) 
    : BTNode(name, BTNodeType::SEQUENCE) {
}
...
BTSelector::BTSelector(const std::string& name) 
    : BTNode(name, BTNodeType::SELECTOR) {
}
...
// BTParallel 구현
BTParallel::BTParallel(const std::string& name, Policy policy) 
    : BTNode(name, BTNodeType::PARALLEL), policy_(policy) {
}
...
// BTRandom 구현
BTRandom::BTRandom(const std::string& name) 
    : BTNode(name, BTNodeType::RANDOM) {
}
...
// BTInvert 구현
BTInvert::BTInvert(const std::string& name) 
    : BTNode(name, BTNodeType::INVERT) {
}
...
void MonsterAI::update(float delta_time) {
    ...
    if (update_counts[monster_name] % 100 == 0) { // 10초마다 로그 출력
        std::cout << "MonsterAI::update 호출됨: " << monster_name << " (카운트: " << update_counts[monster_name] << ")" << std::endl;
    }
    
    if (!active_.load() || !behavior_tree_) {
        if (update_counts[monster_name] % 100 == 0) {
            std::cout << "MonsterAI::update - active: " << active_.load() << ", behavior_tree: " << (behavior_tree_ ? "있음" : "없음") << std::endl;
        }
        return;
    }
