...
namespace bt
{
    ...
    SendBuffer::SendBuffer() : head_(nullptr), tail_(nullptr), total_data_size_(0), read_offset_(0) {}
    ...
    size_t SendBuffer::AppendData(const void* data, size_t size)
    {
        if (size == 0 || !data)
        {
            return 0;
    ...
    bool SendBuffer::HasEnoughData(size_t required_size) const
    {
        return total_data_size_ >= required_size;
    }

    size_t SendBuffer::ExtractData(void* dest, size_t size)
    {
    ...
    size_t SendBuffer::PeekData(void* dest, size_t size) const
    {
        if (size == 0 || total_data_size_ < size || !dest)
        {
            return 0;
    ...
    bool SendBuffer::HasSpace() const
    {
    ...
    size_t SendBuffer::GetUsedSize() const
    {
        return total_data_size_;
    }

    size_t SendBuffer::GetFreeSize() const
    {
    ...
    void SendBuffer::Clear()
    {
        ...
        while (current)
        {
            ...
            SendBufferPool::Instance().Free(current);
            current = next;
        }
    ...
    void SendBuffer::PrintDebugInfo() const
    {
        std::cout << "SendBuffer Debug Info:" << std::endl;
    ...
    void SendBuffer::_AddNewNode()
    {
        auto new_node = SendBufferPool::Instance().Alloc();
    ...
    void SendBuffer::_CleanupEmptyNodes()
    {
        ...
        while (head_ && read_offset_ >= head_->used_size)
        {
            ...
            // 노드를 풀에 반환
            SendBufferPool::Instance().Free(old_head);
        }
    }

    size_t SendBuffer::_ReadFromNode(std::shared_ptr<SendBufferNode> node, size_t offset, void* dest, size_t size) const
    {
