#include "test_client.h"
#include <iostream>
#include <string>
#include <thread>
#include <chrono>

namespace bt {

void print_usage(const char* program_name) {
    std::cout << "BT MMORPG 서버 테스트 클라이언트\n";
    std::cout << "사용법: " << program_name << " [옵션]\n\n";
    std::cout << "옵션:\n";
    std::cout << "  --host <호스트>        서버 호스트 (기본값: 127.0.0.1)\n";
    std::cout << "  --port <포트>          서버 포트 (기본값: 8080)\n";
    std::cout << "  --test <테스트명>      실행할 테스트\n";
    std::cout << "    연결 테스트: connection\n";
    std::cout << "    로그인 테스트: login\n";
    std::cout << "    이동 테스트: move\n";
    std::cout << "    채팅 테스트: chat\n";
    std::cout << "    연결해제 테스트: disconnect\n";
    std::cout << "    전체 테스트: all\n";
    std::cout << "    스트레스 테스트: stress\n";
    std::cout << "  --stress-connections <수>  스트레스 테스트 연결 수 (기본값: 10)\n";
    std::cout << "  --stress-duration <초>     스트레스 테스트 지속 시간 (기본값: 30)\n";
    std::cout << "  --username <이름>       로그인 테스트용 사용자명 (기본값: testuser)\n";
    std::cout << "  --password <비밀번호>   로그인 테스트용 비밀번호 (기본값: testpass)\n";
    std::cout << "  --verbose              상세 로그 출력\n";
    std::cout << "  --help                 이 도움말 표시\n\n";
    std::cout << "예시:\n";
    std::cout << "  " << program_name << " --test all\n";
    std::cout << "  " << program_name << " --test stress --stress-connections 50\n";
    std::cout << "  " << program_name << " --test login --username myuser --password mypass\n";
}

} // namespace bt

int main(int argc, char* argv[]) {
    using namespace bt;
    
    // 기본 설정
    ClientConfig config;
    std::string test_type = "all";
    std::string username = "testuser";
    std::string password = "testpass";
    int stress_connections = 10;
    int stress_duration = 30;
    bool verbose = false;
    
    // 명령행 인수 파싱
    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];
        
        if (arg == "--host" && i + 1 < argc) {
            config.server_host = argv[++i];
        } else if (arg == "--port" && i + 1 < argc) {
            config.server_port = std::stoi(argv[++i]);
        } else if (arg == "--test" && i + 1 < argc) {
            test_type = argv[++i];
        } else if (arg == "--stress-connections" && i + 1 < argc) {
            stress_connections = std::stoi(argv[++i]);
        } else if (arg == "--stress-duration" && i + 1 < argc) {
            stress_duration = std::stoi(argv[++i]);
        } else if (arg == "--username" && i + 1 < argc) {
            username = argv[++i];
        } else if (arg == "--password" && i + 1 < argc) {
            password = argv[++i];
        } else if (arg == "--verbose") {
            verbose = true;
        } else if (arg == "--help") {
            print_usage(argv[0]);
            return 0;
        } else {
            std::cerr << "알 수 없는 옵션: " << arg << std::endl;
            print_usage(argv[0]);
            return 1;
        }
    }
    
    std::cout << "=== BT MMORPG 서버 테스트 클라이언트 ===\n";
    std::cout << "서버: " << config.server_host << ":" << config.server_port << "\n";
    std::cout << "테스트: " << test_type << "\n";
    if (verbose) {
        std::cout << "상세 로그: 활성화\n";
    }
    std::cout << "\n";
    
    // 테스트 클라이언트 생성
    TestClient client(config);
    client.set_verbose(verbose);
    
    bool test_result = false;
    
    try {
        if (test_type == "connection") {
            test_result = client.test_connection();
        } else if (test_type == "login") {
            test_result = client.test_login(username, password);
        } else if (test_type == "move") {
            test_result = client.test_player_move(100.0f, 200.0f, 300.0f);
        } else if (test_type == "chat") {
            test_result = client.test_chat_message("안녕하세요! 테스트 메시지입니다.");
        } else if (test_type == "disconnect") {
            test_result = client.test_disconnect();
        } else if (test_type == "all") {
            test_result = client.run_automated_test();
        } else if (test_type == "stress") {
            test_result = client.run_stress_test(stress_connections, stress_duration);
        } else {
            std::cerr << "알 수 없는 테스트 타입: " << test_type << std::endl;
            print_usage(argv[0]);
            return 1;
        }
    } catch (const std::exception& e) {
        std::cerr << "테스트 실행 중 오류 발생: " << e.what() << std::endl;
        return 1;
    }
    
    std::cout << "\n=== 테스트 완료 ===\n";
    std::cout << "결과: " << (test_result ? "성공" : "실패") << std::endl;
    
    return test_result ? 0 : 1;
}
