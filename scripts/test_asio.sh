...
# BT MMORPG 서버 테스트 스크립트 (Boost.Asio)
...
CLIENT_BINARY="build/bin/AsioTestClient"
SERVER_PORT=8080
TEST_TYPE="all"
VERBOSE=false
...
run_all_tests() {
    ...
    # 2. 몬스터 스폰 테스트
    total_tests=$((total_tests + 1))
    if run_test "몬스터 스폰" "--test spawn --monster-name test_goblin --monster-type 0"; then
        passed_tests=$((passed_tests + 1))
    else
        failed_tests=$((failed_tests + 1))
    fi
    
    # 3. Behavior Tree 실행 테스트
    total_tests=$((total_tests + 1))
    if run_test "BT 실행" "--test bt --bt-name goblin_bt"; then
        passed_tests=$((passed_tests + 1))
    else
        failed_tests=$((failed_tests + 1))
    fi
    ...
    # 4. 몬스터 업데이트 테스트
    total_tests=$((total_tests + 1))
    if run_test "몬스터 업데이트" "--test update"; then
        passed_tests=$((passed_tests + 1))
    else
        failed_tests=$((failed_tests + 1))
    fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --verbose)
            ...
            ;;
        --help)
            echo "BT MMORPG 서버 테스트 스크립트 (Boost.Asio)"
            echo ""
            echo "사용법: $0 [옵션]"
            echo ""
            echo "옵션:"
            ...
            echo "    spawn                   몬스터 스폰 테스트"
            echo "    bt                      Behavior Tree 실행 테스트"
            echo "    update                  몬스터 업데이트 테스트"
...
# 메인 실행
print_info "BT MMORPG 서버 테스트 시작 (Boost.Asio)"
...
case $TEST_TYPE in
    ...
    "connection")
        ...
        ;;
    "spawn")
        run_test "몬스터 스폰" "--test spawn --monster-name test_goblin --monster-type 0"
        ;;
    "bt")
        run_test "BT 실행" "--test bt --bt-name goblin_bt"
        ;;
    "update")
        run_test "몬스터 업데이트" "--test update"
        ;;
    "disconnect")
